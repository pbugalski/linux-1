					...
					struct atmel_smc_cs_conf *smcconf)
{
	 ...
	 *
	 * NWE_SETUP = max(tCLS, tCS, tALS, tDS) - tWP
	 */
	timeps = max3(conf->timings.sdr.tCLS_min, conf->timings.sdr.tCS_min,
		      conf->timings.sdr.tALS_min);
	timeps = max(timeps, conf->timings.sdr.tDS_min);
	timeps = timeps > conf->timings.sdr.tWP_min ?
		 timeps - conf->timings.sdr.tWP_min : 0;
	 ...
	 *
	 * NWE_HOLD = max(tCLH, tCH, tALH, tDH)
	 */
	...
	/*
	 * TDF timing directly matches the tRHZ timing:
	 *
	 * TDF = tRHZ
	 */
