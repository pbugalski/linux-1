...
#define show(x) printk(#x ": 0x%08x [%s:%i]\n", \
		       readl(nfc->regs + x), __func__, __LINE__)
#define showif(x) {							\
		if (probed)						\
			printk(#x ": 0x%08x [%s:%i]\n",			\
				readl(nfc->regs + x),			\
...
"%02x %02x %02x %02x %02x %02x %02x %02x | %02x %02x %02x %02x %02x %02x %02x %02x\n",
		       ...
		       buf[i+6], buf[i+7], buf[i+8], buf[i+9], buf[i+10],
		       buf[i+11], buf[i+12], buf[i+13], buf[i+14], buf[i+15]);
}
...
struct marvell_nfc {
	...
	/* Buffers handling */
	u8 buf[FIFO_GRANULARITY] __aligned(4);
	int buf_pos;
};
 ...
 */
static int marvell_nfc_prepare_cmd(struct mtd_info *mtd)
{
...
static void marvell_nfc_send_cmd(struct mtd_info *mtd, u32 ndcb0, u32 ndcb1,
				 u32 ndcb2, u32 ndcb3)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
...
static int marvell_nfc_wait_ndrun(struct mtd_info *mtd)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct marvell_nfc *nfc = to_marvell_nfc(nand->controller);
	u32 val;
...
static int marvell_nfc_end_cmd(struct mtd_info *mtd, int flag,
			const char *label)
{
	struct nand_chip *nand = mtd_to_nand(mtd);
	struct marvell_nfc *nfc = to_marvell_nfc(nand->controller);
...
static irqreturn_t marvell_nfc_isr(int irq, void *dev_id)
{
	 ...
	 * bit in NDSR (RDY[cs0] and RDY[cs1]).
	 */
	if (st & NDSR_RDY(1))
		st |= NDSR_RDY(0);
...
static void marvell_nfc_read_buf(struct mtd_info *mtd, uint8_t *buf, int len)
{
	...
	if (nfc->buf_pos) {
		i = min(len, FIFO_GRANULARITY - nfc->buf_pos);
		memcpy(buf, &nfc->buf[nfc->buf_pos], i);
		nfc->buf_pos = (nfc->buf_pos + i) % FIFO_GRANULARITY;
	...
	while (i < len) {
		 ...
		 */
		step_len = min_t(int, len - i, FIFO_GRANULARITY);

		ioread32_rep(nfc->regs + NDDB, (u32 *)&nfc->buf,
			FIFO_GRANULARITY / 4);
				...
				int len)
{
	...
	u8 fifo[FIFO_GRANULARITY] __aligned(4);
	...
	while (i < len) {
		 ...
		 * - The chunk to write cannot be smaller than FIFO_GRANULARITY
		 * Remaining bytes indicates the number of bytes that will
		 * remain in the last uncomplete write (less than
		 * FIFO_GRANULARITY bytes to write). They will then be copied in
		...
		if (distance >= MAX_CHUNK) {
			...
			rounded_len = round_down(distance, FIFO_GRANULARITY);
			remaining_bytes = distance - rounded_len;
		}

		/* write_length matches the number of data cycles */
		if (remaining_bytes)
			write_length = rounded_len + FIFO_GRANULARITY;
		else
		 ...
		 */
		for (j = 0; j < rounded_len / FIFO_GRANULARITY; j++) {
			memcpy(fifo, &buf[i + (j * FIFO_GRANULARITY)],
				FIFO_GRANULARITY);
			iowrite32_rep(nfc->regs + NDDB, fifo,
				FIFO_GRANULARITY / 4);
		}
		...
		if (remaining_bytes) {
			 ...
			 * FIFO_GRANULARITY, pad with empty bytes: 0xFF.
			 */
			memset(fifo, 0xFF, FIFO_GRANULARITY);
					    ...
					    int page)
{
	...
	struct marvell_nand_chip *marvell_nand = to_marvell_nand(nand);
	struct marvell_nfc *nfc = to_marvell_nfc(nand->controller);
	struct marvell_hw_ecc_layout *lt = &to_marvell_nand(nand)->layout;
	int max_bitflips = 0, ret = 0;
	u8 *data, *oob;
	...
	/*
	 * Allocating a dummy OOB buffer may be needed because ->write_oob()
	 * must call ->write_page() (it is not possible to write OOB data
	 * without writing the whole page with ECC enabled) and in order to
	 * rewrite the page correctly, ->read_page() must be called too to
	 * retrieve the data to write. ->read_page() has to read OOB data and
	 * store it somewhere else than in chip->oob_poi where the actual OOB
	 * data to be finally written remains.
	 */
	data = buf;
	if (oob_required) {
		oob = chip->oob_poi;
	...
	/* Read the data, then jump to the spare bytes and read it */
	ioread32_rep(nfc->regs + NDDB, data, lt->data_bytes / 4);
	ioread32_rep(nfc->regs + NDDB, oob, lt->spare_bytes / 4);

	marvell_nfc_hw_ecc_correct(mtd, data, lt->data_bytes,
...
static void marvell_nfc_hw_ecc_bch_read_chunk(struct mtd_info *mtd, int chunk,
					      u8 *data, int data_len,
					      u8 *oob, int oob_len,
	...
	for (chunk = 0; chunk < nchunks; chunk++) {
		if (chunk == 0) {
			...
			if (oob_required) {
				...
				/*
				 * Justification for this allocation is
				 * available in ->hmg_read_page().
				 */
				const_oob = kmalloc(mtd->oobsize, GFP_KERNEL);
				oob = const_oob;
				if (!oob)
					return -ENOMEM;
			}
		...
		marvell_nfc_hw_ecc_bch_read_chunk(mtd, chunk, data, data_len,
				       ...
				       struct nand_chip *chip, int page)
{
	...
	dummy_data = kmalloc(mtd->writesize, GFP_KERNEL);
	if (!dummy_data)
		return -ENOMEM;

	ret = chip->ecc.read_page(mtd, chip, dummy_data, true, page);
	kfree(dummy_data);
					     ...
					     int oob_required, int page)
{
	...
	/* The core already sent a ->cmdfunc(NAND_CMD_SEQIN) but the "types"
	 * fields were missing, so send it again here. No need for a double byte
	 * command with NAND_CMD_PAGEPROG as long as the core will trigger
	 * right after the ->cmdfunc(NAND_CMD_PAGEPROG) and then ->waitfunc().
	 *
	 * If we add the NAND_CMD_PAGEPROG aside, the second NAND_CMD_PAGEPROG
	 * sent by the core will lead to a timeout on RB polling while there was
	 * actually no error.
	 */
	ret = marvell_nfc_prepare_cmd(mtd);
	if (ret)
	...
	iowrite32_rep(nfc->regs + NDDB, data, lt->data_bytes / 4);
	iowrite32_rep(nfc->regs + NDDB, oob, lt->spare_bytes / 4);
...
static void marvell_nfc_hw_ecc_bch_write_chunk(struct mtd_info *mtd, int chunk,
					       ...
					       int oob_required, int page)
{
	...
	struct marvell_nfc *nfc = to_marvell_nfc(nand->controller);
	struct marvell_hw_ecc_layout *lt = &to_marvell_nand(nand)->layout;
	int data_len, oob_len, write_len, xtype;
					     ...
					     int oob_required, int page)
{
	...
	for (chunk = 0; chunk < nchunks; chunk++) {
		marvell_nfc_hw_ecc_bch_write_chunk(mtd, chunk, buf,
						   chip->oob_poi,
						   oob_required, page);
		/*
					...
					struct nand_chip *chip, int page)
{
	...
	chip->ecc.read_page(mtd, chip, data, false, page);

	return chip->ecc.write_page(mtd, chip, data, true, page);
}
				...
				struct device_node *np)
{
	...
	if (!of_get_property(np, "marvell,nand-keep-config", NULL))
		nand->setup_data_interface = marvell_nfc_setup_data_interface;
	...
	WARN_ONCE(nand->options & NAND_BUSWIDTH_16,
		"NAND: using untested 16-bit bus width topology\n");
	...
	if ((mtd->writesize <= 512 && (nand->chipsize > (32 << 20))) ||
		(mtd->writesize > 512 && (nand->chipsize > (128 << 20))))
		marvell_nand->addr_cyc = 5;
