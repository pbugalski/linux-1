...
static void fsl_qspi_select_mem(struct fsl_qspi *q, u8 cs)
{
	void __iomem *base = q->iobase;
	int i;
			    ...
			    const struct spi_mem_op *op)
{
	 ...
	 */
	qspi_writel(q, op->data.nbytes, base + QUADSPI_IPCR);
...
static unsigned int fsl_qspi_max_data_bytes(struct spi_mem *mem,
					    enum spi_mem_data_dir dir)
{
	struct fsl_qspi *q = spi_controller_get_devdata(mem->spi->master);

	if (dir == SPI_MEM_DATA_IN)
		return q->devtype_data->rxfifo;

	return q->devtype_data->txfifo;
}
...
static const struct spi_controller_mem_ops fsl_qspi_mem_ops = {
	.max_data_bytes = fsl_qspi_max_data_bytes,
